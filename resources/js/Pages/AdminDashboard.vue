...
<template>
  <Head title="Admin Dashboard" />
    <div class="max-w-6xl mx-auto px-4 py-8">
      ...
      <!-- Filtros -->
      <div v-if="mostrarFiltros" class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6 items-end">
        ...
        <input type="date" v-model="filtroVencimentoInicio" class="border p-2 rounded" placeholder="De">
        <input type="date" v-model="filtroVencimentoFim" class="border p-2 rounded" placeholder="Até">
      </div>

      <!-- Botão de ordenação por vencimento -->
      <div class="mb-6 flex justify-end">
        <button
          @click="ordenarPorVencimentoAsc = !ordenarPorVencimentoAsc"
          class="bg-gray-500 hover:bg-gray-700 text-white px-2 py-2 rounded hover:bg-indigo-700 transition"
        >
          Ordenar: {{ ordenarPorVencimentoAsc ? 'Vence primeiro' : 'Vence por último' }}
        </button>
      </div>
...
export default {
  ...
  computed: {
    tarefasFiltradas() {
      let filtradas = this.tarefas.filter(tarefa => {
      ...
      return filtradas.sort((a, b) => {
        ...
        return this.ordenarPorVencimentoAsc ? aDate - bDate : bDate - aDate
      })
    },
